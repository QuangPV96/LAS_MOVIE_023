...
public enum AddTextViewHandler:Int {
    case close = 0
    case rotate
    case flip
}

public enum AddTextViewPosition:Int {
    case topLeft = 0
    case topRight
    case bottomLeft
    case bottomRight
...
@inline(__always) func AddTextViewCGRectGetCenter(_ rect:CGRect) -> CGPoint {
    return CGPoint(x: rect.midX, y: rect.midY)
}

@inline(__always) func AddTextViewCGRectScale(_ rect:CGRect, wScale:CGFloat, hScale:CGFloat) -> CGRect {
    return CGRect(x: rect.origin.x, y: rect.origin.y, width: rect.size.width * wScale, height: rect.size.height * hScale)
}

@inline(__always) func AddTextViewCGAffineTransformGetAngle(_ t:CGAffineTransform) -> CGFloat {
    return atan2(t.b, t.a)
}

@inline(__always) func AddTextViewCGPointGetDistance(point1:CGPoint, point2:CGPoint) -> CGFloat {
    let fx = point2.x - point1.x
    let fy = point2.y - point1.y
    return sqrt(fx * fx + fy * fy)
}

@objc public  protocol AddTextViewStickerViewDelegate {
    @objc func addTextViewDidBeginMoving(_ stickerView: AddTextView)
    @objc func addTextViewDidChangeMoving(_ stickerView: AddTextView)
    @objc func addTextViewDidEndMoving(_ stickerView: AddTextView)
    @objc func addTextViewDidBeginRotating(_ stickerView: AddTextView)
    @objc func addTextViewDidChangeRotating(_ stickerView: AddTextView)
    @objc func addTextViewDidEndRotating(_ stickerView: AddTextView)
    @objc func addTextViewDidClose(_ stickerView: AddTextView)
    @objc func addTextViewDidTap(_ stickerView: AddTextView)
}

public class AddTextView: UIView {
    public var delegate: AddTextViewStickerViewDelegate!
    ...
    public  init(contentView: UIView) {
        ...
        self.contentView.center = AddTextViewCGRectGetCenter(self.bounds)
    ...
    func handleMoveGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .began:
            ...
            if let delegate = self.delegate {
                delegate.addTextViewDidBeginMoving(self)
            }
        case .changed:
            ...
            if let delegate = self.delegate {
                delegate.addTextViewDidChangeMoving(self)
            }
        case .ended:
            self.center = CGPoint(x: self.beginningCenter.x + (touchLocation.x - self.beginningPoint.x), y: self.beginningCenter.y + (touchLocation.y - self.beginningPoint.y))
            if let delegate = self.delegate {
                delegate.addTextViewDidEndMoving(self)
            }
        default:
            break
        }
    ...
    func handleRotateGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        switch recognizer.state {
        case .began:
            self.deltaAngle = CGFloat(atan2f(Float(touchLocation.y - center.y), Float(touchLocation.x - center.x))) - AddTextViewCGAffineTransformGetAngle(self.transform)
            self.initialBounds = self.bounds
            self.initialDistance = AddTextViewCGPointGetDistance(point1: center, point2: touchLocation)
            if let delegate = self.delegate {
                delegate.addTextViewDidBeginRotating(self)
            }
        case .changed:
            ...
            var scale = AddTextViewCGPointGetDistance(point1: center, point2: touchLocation) / self.initialDistance
            let minimumScale = CGFloat(self.minimumSize) / min(self.initialBounds.size.width, self.initialBounds.size.height)
            scale = max(scale, minimumScale)
            let scaledBounds = AddTextViewCGRectScale(self.initialBounds, wScale: scale, hScale: scale)
            self.bounds = scaledBounds
            self.setNeedsDisplay()
            
            if let delegate = self.delegate {
                delegate.addTextViewDidChangeRotating(self)
            }
        case .ended:
            if let delegate = self.delegate {
                delegate.addTextViewDidEndRotating(self)
            }
        default:
            break
        }
    ...
    func handleCloseGesture(_ recognizer: UITapGestureRecognizer) {
        if let delegate = self.delegate {
            delegate.addTextViewDidClose(self)
        }
    ...
    func handleTapGesture(_ recognizer: UITapGestureRecognizer) {
        if let delegate = self.delegate {
            delegate.addTextViewDidTap(self)
        }
    }
...
extension AddTextView: UIGestureRecognizerDelegate {
