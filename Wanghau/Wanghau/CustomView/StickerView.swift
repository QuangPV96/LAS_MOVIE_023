...
@objc public  protocol StickerViewDelegate {
    ...
    @objc func stickerViewDidChangeRotating(_ stickerView: StickerView, scale: CGFloat)
...
public class StickerView: UIView {
    ...
    func handleRotateGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            print("trungnc scale:\(scale), scaledBounds: \(scaledBounds)")

            self.setNeedsDisplay()
            
            if let delegate = self.delegate {
                delegate.stickerViewDidChangeRotating(self, scale: scale)
            }
        case .ended:
            print("trungnc Rotate ended")
            if let delegate = self.delegate {
